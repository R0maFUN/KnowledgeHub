...
struct client_info *get_or_create_client(SOCKET s)
{
    ...
    // client not found, going to create new one and add it to the clients
...
fd_set wait_on_clients(SOCKET server)
{
    ...
    if (select(max_socket + 1, &reads, 0, 0, 0) < 0) {
        fprintf(stderr, "select() call failed. (%d)", GETSOCKETERRNO());
        exit(1);
    }
...
void serve_resource(struct client_info *client, const char *path)
{
    ...
    // Possible blocking behaviour of send() on large files.
...
int main()
{
    ...
    SOCKET server = create_listen_socket(0, "8080");
    // listen only local connections:
    // SOCKET server = create_listen_socket("127.0.0.1", "8080");
    ...
    while (1) {
        ...
        if (FD_ISSET(server, &reads)) {
            struct client_info *client = get_or_create_client(-1);
